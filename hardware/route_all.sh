...
#   ./route_all.sh
# Net classes (scripts/setup_netclasses.py): Default 0.20mm / Power 0.40mm / Fine 0.15mm (W5500-dense only).
...
unrouted(){ run scripts/validate_connectivity.py 2>>"$L" | grep -oE "unrouted nets: [0-9]+" | grep -oE "[0-9]+$"; }

echo "[1] net classes";           run scripts/setup_netclasses.py | tail -1
echo "[2] planes + stitch vias";  run scripts/rebuild_iso.py >/dev/null; echo "    ok"
echo "[3] connector escapes";     run scripts/escape_connectors.py >/dev/null; echo "    ok"
...
for try in 1 2 3; do
  ...
  FR2_PASSES=$p run scripts/autoroute_fr2.py >>"$L" 2>&1
  u=$(unrouted)
...
if [ "$ok" = "1" ]; then
  ...
  run scripts/finish_partial.py | tail -2                   # FR sometimes leaves a net a pad short (a gap); the maze
                                                    # straggler skips those (they have tracks), so delete them first
  MAZE_INFL=0.24 run scripts/route_tbu.py | grep -E "routed [0-9]+/8 nets|no path to"
fi

echo "[6] pad cleanup";  run scripts/cleanup_pads.py | tail -1
echo "[7] PoE void";     run scripts/tighten_poe_void.py | tail -1
echo "[7b] eth impedance"; run scripts/widen_eth.py | tail -1     # widen the MDI pairs toward ~50 ohm SE (necks at the QFN)
echo "[8] validate";     run scripts/validate_connectivity.py | grep -E "connectivity gate|RESULT"
