...
<div class="container-fluid p-3">
  ...
  <section data-panel="ana" hidden>
    ...
    <div class="card mt-3"><div class="card-header d-flex justify-content-between align-items-center">
      ...
      <div class="row g-3 mb-3">
        <div class="col-6 col-md-3"><div class="metric"><div class="v" id="rc-replies">0</div><div class="k">replies checked</div></div></div>
        <div class="col-6 col-md-3"><div class="metric"><div class="v" id="rc-bad">0</div><div class="k">corrupted replies</div></div></div>
...
function renderMetrics(s,m){
  ...
  if(m.rcBytes!=null){
    const tapped=m.rcBytes>0, checked=m.rcReplies||0, bad=m.rcBadReplies||0;
    $('#rc-replies').textContent=checked;
    $('#rc-bad').textContent=bad; $('#rc-bad').style.color=(bad>0)?CSS('--lux-magenta'):CSS('--lux-green');
    ...
    const clean=tapped && checked>0 && bad===0;
    $('#rc-dot').className='dot '+(!tapped?'off':checked===0?'off':clean?'on':'bad');
    $('#rc-state').textContent=!tapped?'no tap':checked===0?'idle':clean?'clean':'errors';
    const d=diffHex(m.txReplyHex||'', m.rcReplyHex||'');
    $('#rc-sent').innerHTML=d.sent; $('#rc-recv').innerHTML=d.recv;
    const pct=checked>0?Math.round(100*bad/checked):0;
    $('#rc-note').textContent = !tapped
      ...
      : checked===0 ? 'Line is up but no reply captured yet. Trigger a discovery or GET on the controller.'
      : clean ? 'Every reply reached the controller byte-perfect ('+checked+' checked). So any discovery misses are the controller firmware read, not the wire.'
      : bad+' of '+checked+' replies corrupted ('+pct+'%) on the wire. The reply leg has a signal-integrity problem, not the firmware. Highlighted bytes show where it flipped.';
  }
}
