...
| ESP32-S3 with **octal** PSRAM (N8R8 / N16R8) | `esp32s3_psram` | WiFi | 2 | GPIO17 / 16 / −1 | UART2, TX 18 | Enables the 8 MB **octal** PSRAM: RDM tables, WiFi/lwIP buffers and pixel framebuffers move to external RAM, so the RDM cap auto-detects to 64 and the internal heap stays open. Measured on an N16R8: 8 MB found, 121 KB internal heap free against 57 KB with the plain image. **Octal only.** Quad vs octal is fixed at build time on the ESP32-S3, so a module with 2 MB *quad* PSRAM (`R2`) runs the plain `esp32s3dev` image and simply does not use its PSRAM. Folding quad into that image was tried and dropped on purpose: it would change the firmware every S3 in the field auto-updates to, in exchange for a size of PSRAM none of our boards use. PSRAM occupies GPIO33-37, DMX on 16/17 is clear |
