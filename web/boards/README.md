...
`luxdmx_v4` is a **previous revision of our own board**, kept here as a legacy descriptor.
The current revision is `luxdmx_v6` (that is what the template and `BOARD_ID` say). It is not
a built-in, so it costs no flash.
