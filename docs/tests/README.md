...
| `pixels.spec.mjs` | WS281x pixel output: `/pixels.json` shape, the frame-rate ceiling matches the wire, a disabled port allocates nothing, the settings page renders a card per port and states the universe span + next free universe. Behavioural (opt-in): Art-Net lands byte-exact in the framebuffer, a port spanning two universes stitches them in order, sACN feeds a port identically, the power cap scales and reports its factor, a missing universe latches partially instead of dropping the frame, and the backend budget: pixel ports share the RMT transmit channels with DMX TX and a WS2812 status LED, so the test derives how many ports still fit from `rmtMax` minus the enabled outputs, asserts that many give `rmt` and one more gives `lcd` while still clocking frames, and that `pixbk=2` forces LCD_CAM where RMT would have been picked (skipped on a board that leaves no testable step, and on one without PSRAM whose largest free block is too small) |
