...
| `pixels.spec.mjs` | WS281x pixel output: `/pixels.json` shape, the frame-rate ceiling matches the wire, a disabled port allocates nothing, the settings page renders a card per port and states the universe span + next free universe. Behavioural (opt-in): Art-Net lands byte-exact in the framebuffer, a port spanning two universes stitches them in order, sACN feeds a port identically, the power cap scales and reports its factor, a missing universe latches partially instead of dropping the frame, and the 4th pixel port switches the driver from RMT to LCD_CAM and still clocks frames on every port (that last one skips itself on a board with fewer than 5 ports, or on one without PSRAM whose largest free block is too small to hold them safely) |
| `multi-output.spec.mjs` | Issue #4: `outputs[3]` shape, migration, RDM binding, UI (Output A/B/C); one TX pin per enabled output; splitter + pin-less-output regression (opt-in) |
