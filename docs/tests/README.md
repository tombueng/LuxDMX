...
| `output-rate.spec.mjs` | Per-output DMX transmit rate + style (issue #93): `/info.json` carries `rate`/`style`/`styleSrc` plus a per-output beat flag (`beat`) and detected change rate (`chgHz`); `/config` renders both selectors including the 33.3 fps entry and the provenance badge; ArtPollReply `RefreshRate` tracks the configured rate and `GoodOutputB` bit6 the configured style; the navbar renders one C/D letter per output with a dot marking a style a controller pushed over Art-Net (and a ⚠ marking a beat risk), and spells it out in the tooltip. Behavioural (opt-in): picking 33.3 fps really moves the transmit rate off 40, Delta makes the output follow a deliberately slow source instead of free-running, Delta tracks the real change rate rather than the packet rate when the source repeats each frame 3x (the MagicQ-burst case that reopened #93), a Continuous output beating a rate-mismatched source raises the beat warning while Delta and a rate-matched source clear it (reading the ~33 Hz *change* rate, not the ~100 packets/s), and a silent source in Delta falls back to the configured rate so the line never stops. The duplicate-frame share, and the proof that the rate arrives on the *wire* rather than merely being scheduled, are in `dmx-wire.spec.mjs`. |
...
| `artnet-rdm.spec.mjs` | RDM over Art-Net (E1.20 over Art-Net 4): `/rdm.json` node fields (`artnetRdm`/`artPort`/`discovering`/`bqPolicy` + request counters); drives the device as an Art-Net RDM controller: ArtPoll → ArtPollReply (asserts RDM-capable + BackgroundQueue-supported advertisement), ArtTodRequest → ArtTodData (TOD matches `/rdm.json`), and (opt-in) ArtRdm GET DEVICE_INFO + SET DMX_START_ADDRESS with read-back, AtcFlush re-discovery, a check that a mid-stream flush doesn't collapse the transmit rate (`outfps`), and `ArtAddress` remote config (merge HTP/LTP/cancel applied live; BackgroundQueuePolicy set + reflected in `/rdm.json` and ArtPollReply). Also pins the ArtPollReply timing advertisement honest (issue #93): `RefreshRate` matches the measured `outfps` and stays ≤ 44 Hz, `GoodOutputB` bit6 reports the continuous (free-running) output style, and `Status3` bits7-6 mirror the port’s signal-loss policy. The DMX-keeps-clocking-under-RDM proof is in `rdm-wire.spec.mjs`, measured on the wire. Also (opt-in) pins the universe-renumber path: after changing an output's universe the TOD answers under the new port-address immediately (no re-scan, no reboot), nothing answers on the vacated one, and the device table is re-stamped rather than left with orphans eating the device cap. |
| `artnet-address.spec.mjs` | Full `ArtAddress` (0x6000) remote configuration (issue #129), the thing a console's own network window (MagicQ Network Manager, DMX-Workshop) uses. Read-only: `ArtPollReply` serves the configured node names and falls back to the **hostname** rather than the old hard-coded "LuxDMX" that made every box on a rig look identical in a node list; one reply per enabled output, each with its own 1-based `BindIndex`, whose `NetSwitch`/`SubSwitch`/`SwOut` split recomposes to exactly that output's universe in `/info.json`; and an `ArtAddress` carrying only a command (no name bytes, no bit-7 address bytes) changes nothing, which is the regression that matters most here (without the bit-7 rule a console flipping the merge mode drags the port to universe 0). Opt-in (`LUXDMX_WRITE=1`): a rename over the wire lands in `artshort`/`artlong`, is echoed in the confirming reply and a later poll, and leaves `hostname` (mDNS / DHCP name) alone; an empty name field does not blank a name already set; programming a port-address moves that output live, only that output, reads back in the reply and `/info.json`, survives a reboot, and sACN on the new universe (+1) then drives that output. Restores the whole config afterwards. Note: sACN is injected **unicast** like everywhere else in this suite, so that last one proves the routing followed the change, not that the multicast group was re-joined — multicast does not reach the DUT across the bench, and the `beginMulticast()` the rejoin flag triggers is only visible on the device's serial log. |
...
| `config-guard.spec.mjs` | The `/config` page never invents settings (issue #132). It renders from `/info.json`; when that answer is not this device's config (half-booted device, error body, captive portal, `outputs` missing or not an array) the page must fall into its "Cannot read settings" state with **Save disabled**, instead of substituting the hardcoded "output A on GPIO17, output B off" placeholders with Save still live -- one click of which wrote those invented pins over a working setup, while the device kept running on its real pins until the next reboot. Read-only: each case intercepts `/info.json` in the browser, so the device is never written to. |
