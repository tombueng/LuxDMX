...
| *(bench only, not Playwright)* `tear_test.py` | Proves the DMX output frame is never read while it is being written. Fills all 512 channels with one value, so a torn frame shows as two values with a split point, and sweeps the input rate. A/B against the pre-fix firmware: 8.81% torn at 44 fps in before the seqlock, 3.19% after, with the residual being the analyzer's own unlocked DMA read (it is already there at 5 fps, where the write race barely exists). Lives on the RP2350 rig, not here, because it needs the analyzer on the DMX line. |
| `signal-loss.spec.mjs` | Per-output signal-loss policy: `/info` loss field + `/config` selector; after the 4 s source timeout HOLD keeps the frame, BLACKOUT zeros it, STOP holds (not zero), over Art-Net + sACN; persistence across reboot (opt-in) |
