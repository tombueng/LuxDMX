...
**34 boards** spanning every esp32 / esp32s3 board the firmware runs on (see
...
- **Radig ESP32-S3 DMX/RDM Node** is a carrier for a XIAO ESP32-S3 that already has the
  RS-485 driver and the XLR on it, so it borrows the XIAO pin columns and marks its four
  fixed pins (DMX TX 4 / RX 5, DE/RE 6, WS2812 1) as hardwired. Its preset sets `rts`, so
  RDM works out of the box, unlike the bare dev boards that leave DE/RE unwired (`-1`).
