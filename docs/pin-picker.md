...
- **LuxDMX v6** is parsed straight from `hardware/luxdmx.py` (the PCB netlist source),
  so its diagram, template and Ethernet-reserved-pin rules cannot drift from the board.
  (That file still carries the board's v5.x revision markings; the hardware sources have not
  been renumbered, only the firmware/web-facing board name has.)
