...
struct Config {
    ...
    // build *reports* (BOARD_ID in main.cpp) is compile-time. A LuxDMX v6 runs the plain
    // esp32s3dev build and reports "esp32s3-devkitc-1", so without this the selector fell
    // back to the DevKit on every reboot / OTA while the applied pins stayed put.
    String    boardSel;
    int       protocol;
    int       ledPin;
    int       ledType;
